...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    rf1_out, rf1_cache = relu_forward(af1_out)
    af2_out, af2_cache = affine_forward(rf1_out, W2, b2)
    #rf2_out, rf2_cache = relu_forward(af2_out)
    scores = af2_out
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    daf2_out, dW2, db2 = affine_backward(dscores, af2_cache)
    drf1_out = relu_backward(daf2_out, rf1_cache)
    daf1_out, dW1, db1 = affine_backward(drf1_out,af1_cache)
    dW2 += self.reg * W2
    dW1 += self.reg * W1
    grads['W1'] = dW1
    grads['b1'] = db1
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(num_layers-1):
      if not self.use_batchnorm:
        ...
        out[i+1], cache[i+1] = affine_bn_relu_forward(out[i], W, b, gamma, beta, self.bn_params[i])
    ...
    out[num_layers], cache[num_layers] = affine_forward(out[num_layers-1], W, b)
    #if self.use_dropout:
    #  out[num_layers], dropout_cache[num_layers] = dropout_forward(out[num_layers], self.dropout_param)
    ...
    dout[num_layers], dW[num_layers], db[num_layers] = affine_backward(dscores,cache[num_layers])
    
    #dout[num_layers] = dropout_backward(dout[num_layers],dropout_cache[num_layers]) 
    grads['W'+str(num_layers)] = dW[num_layers] + reg * self.params['W'+str(num_layers)]
    grads['b'+str(num_layers)] = db[num_layers]
    for i in xrange(num_layers-1,0,-1):
      if self.use_dropout:
        dout[i+1] = dropout_backward(dout[i+1],dropout_cache[i])

      if not self.use_batchnorm:  
        dout[i], dW[i], db[i] = affine_relu_backward(dout[i+1], cache[i])
        dW[i] += reg * self.params['W'+str(i)]
        grads['W'+str(i)] = dW[i]
        grads['b'+str(i)] = db[i]
      else:
        dout[i], dW[i], db[i], dgamma[i], dbeta[i] = affine_bn_relu_backward(dout[i+1], cache[i])
        dW[i] += reg * self.params['W'+str(i)]
        grads['W'+str(i)] = dW[i]
        grads['b'+str(i)] = db[i]
